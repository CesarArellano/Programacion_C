...
{
  int ID_Biciestacion, LugaresLlenos, LugaresDisponibles, Activo;
  char Nombre[200], CalleNumero[200], Ciudad[200], CP[50];
...
typedef struct Def_Usuario //Estructura del Usuario
{
  ...
  char Nombre[200], Password [200], CalleNumero [200], NumeroTarjeta[50];
...
void MenuUsuario(TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta *PrimeraBicicleta, TipoUsuario *PrimerUsuario, int Numeracion [3], int ID_Sesion); //Esta función imprime el menú principal del usuario y lo redirige a sus respectivas funciones.
      ...
      if(argc==1) // Cuando el usuario, solo inserta el ejecutable puede acceder al sistema.
    {
      ...
      do
        ...
        { 
          ...
          if (Flag==0 && TipoUsu==0)
        {
          MenuUsuario (PrimeraBiciestacion, PrimeraBicicleta, PrimerUsuario,Numeracion,ID_Sesion);
          SeLogueo = 1;
        }
          else
        {
...
void MenuAdmin (TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta *PrimeraBicicleta, TipoUsuario *PrimerUsuario, int Numeracion [3], int ID_Sesion) //Este menú imprime el menu del administrador y lleva al mismo a sus respectivas herramientas.
{
  ...
  do {
    ...
    printf ("Calle y Número de la biciestación: ");
      ...
      ErrorLectura = 1;
    printf ("Ingrese el codigo postal [5 dígitos]: ");
    if ((scanf (" %[^\n]", NuevaBiciestacion.CP)) == 0)
      ErrorLectura = 1;
      CodigoP=strlen(NuevaBiciestacion.CP);
      if(CodigoP==5)
      {
        CodigoP = 0;
        for(int i=0; i<5; i++)
        {
          if(NuevaBiciestacion.CP[i]>=48 && NuevaBiciestacion.CP[i]<=57)
          {
           CodigoP += 1;
          }
          else
    ...
    if (ErrorLectura == 0) //Si no hubo error, se le pide al administrador que confirme el guardado de datos.
      {
        ...
        printf("Código postal de la biciestación: %s\n",NuevaBiciestacion.CP);
    ...
    printf ("Contraseña del nuevo usuario: ");
      ...
      ErrorLectura = 1;
    printf ("Numero de tarjeta de credito del nuevo usuario [16 dígitos]: ");
    if ((scanf (" %[^\n]", NuevoUsuario.NumeroTarjeta)) == 0)
      ErrorLectura = 1;
    NumeroT=strlen(NuevoUsuario.NumeroTarjeta);
      if(NumeroT==16)
      {
        NumeroT = 0;
        for(int i=0; i<16; i++)
        {
          if(NuevoUsuario.NumeroTarjeta[i]>=48 && NuevoUsuario.NumeroTarjeta[i]<=57)
          {
           NumeroT += 1;
          }
          else
          ...
          while(PrimeraBiciestacion!=NULL && *ErrorEstacionar!=1) //Hasta encontrar NULL
        {
          if((strcmp(PrimeraBiciestacion->Nombre,NuevaBiciestacion.Nombre))==0) //Saber si la biciestacion existe
              ...
              if(PrimeraBiciestacion->LugaresDisponibles>0) //Hay lugares vacios para estacionar
            {
              ...
              if (*ErrorEstacionar != 1)
                {
                  *ErrorEstacionar=4; //Acabó de recorrer las bicicletas pero no rentó ninguna con exito, se devuelve el Error 4.
                }
            }
              else
              {
                *ErrorEstacionar=4; //No hay lugares disponibles, se devuelve el Error 5.
              }
            }
          else // No existe biciestacion
            {
...
void AltaBiciestaciones(TipoBiciestacion **PrimeraBiciestacion, TipoBiciestacion NuevaBiciestacion, int Numeracion [3], int ID_Sesion) //Esta función inserta al final de la lista dinámica de Biciestaciones la biciestación que se leyó en "MenuAdmin".
{
  ...
  Numeracion [0] ++;
  strcpy(nuevo -> CP, NuevaBiciestacion.CP);
...
void ReasignarBicicletas(TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta *PrimeraBicicleta,TipoBicicleta NuevaBicicleta, TipoBiciestacion NuevaBiciestacion, int *Verificar_Reasignacion, int ID_Sesion) //Esta función verifica si la Bicicleta que se especificó en "MenuAdmin" se puede mover a la Biciestación que se especificó en "MenuAdmin", en ese caso, se resta un lugar vacio a la Biciestación de destino, un lugar lleno a la Biciestación de origen, se suma un lugar lleno a la Biciestación de destino, un lugar vacío a la Biciestación de origen y se le cambia el ID de Biciestación a la Bicicleta.
{
  ...
  while (PrimeraBicicleta!=NULL && *Verificar_Reasignacion!=1)
    {
      Entro=1;
      if(PrimeraBicicleta->ID_Bicicleta==NuevaBicicleta.ID_Bicicleta) //Encuentra si el ID de la Bicicleta se encuentra en la lista dinámica.
    {
      if((strcmp("NULL",PrimeraBicicleta->Timestamp))==0) //Comprueba si la bicicleta está ocupada.
        ...
        *Verificar_Reasignacion=0; // Esta ocupada la bicicleta.
    }
      else 
      {
        if(*Verificar_Reasignacion!=0)
        *Verificar_Reasignacion=0; //No se encontró el ID de la bicicleta.
      }
      PrimeraBicicleta=PrimeraBicicleta->sig;
    }
...
void AltaUsuario(TipoUsuario **PrimerUsuario, TipoUsuario NuevoUsuario, int Numeracion [3], int ID_Sesion) //Esta función inserta al final de la lista dinámica de Usuarios al usuario que haya leído.
{
  ...
  strcpy (nuevo -> NumeroTarjeta, NuevoUsuario.NumeroTarjeta);
...
void AltaPrimerUsuario (TipoUsuario **PrimerUsuario, int *PrimeraEjecucion, int Numeracion [3]) //Esta funcion da de alta al primer usuario como administrador.
{
  ...
  int ErrorLectura, ID_Sesion,NumeroTag;
  char Enter;
  system ("clear");
  printf ("\n");
  Logo ();
  ...
  do
    ...
    ErrorLectura = 1;
      ...
      if ((scanf (" %[^\n]", NuevoUsuario.NumeroTarjeta)) == 0)
      ErrorLectura = 1;
    NumeroTag=strlen(NuevoUsuario.NumeroTarjeta);
      if(NumeroTag==16)
      {
        NumeroTag = 0;
        for(int i=0; i<16; i++)
